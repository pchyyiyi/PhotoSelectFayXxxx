...
/**
 * @ClassName GlideRoundTransformation
 * @Description TODO
 * @Author fayXxxx
 * @Date 2021/5/20 下午5:57
 * @Version 1.0
 */
public class GlideRoundCornerTransform extends BitmapTransformation {
    private static final int VERSION = 1;
    //这个ID值 随意
    private static final String ID = "com.xwdd.studytestservice.widget.GlideRoundCornerTransform." + VERSION;
    ...
    public GlideRoundCornerTransform() {
        this(6, 0, CORNER_ALL, FIT_CENTER);
    }

    public GlideRoundCornerTransform(int dpRadius, int marginDp) {
        this(dpRadius, marginDp, CORNER_ALL, FIT_CENTER);
    }

    public GlideRoundCornerTransform(int dpRadius, int marginDp, int cornerType, @ScaleType int scaleType) {
    ...
    public String toString() {
        return "GlideRoundCornerTransform(radius=" + radius + ", margin=" + margin + ", diameter="
                + diameter + ", cornerType=" + cornerType + ")";
    }

    @Override
    public boolean equals(Object o) {
        return o instanceof GlideRoundCornerTransform &&
                ((GlideRoundCornerTransform) o).radius == radius &&
                ((GlideRoundCornerTransform) o).diameter == diameter &&
                ((GlideRoundCornerTransform) o).margin == margin &&
                ((GlideRoundCornerTransform) o).cornerType == cornerType;
    }
